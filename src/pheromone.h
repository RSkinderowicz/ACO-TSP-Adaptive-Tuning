...
struct CandListPheromone {
    ...
    [[nodiscard]] double get_ith(uint32_t from, uint32_t index) const {
        assert((from < dimension_) && (from < dimension_));
        assert(index < cl_size_);

        auto offset = from * cl_size_ + index;
        return trails_[offset];
    }
    ...
    void evaporate_and_deposit(double evaporation_rate,
                               double min_pheromone_value, double max_pheromone_value,
                               double delta,
                               uint32_t src_node,
                               uint32_t node_a, uint32_t node_b) {

        const auto offset = src_node * cl_size_;

        for (uint32_t i = offset; i < offset + cl_size_; ++i) {
            auto node = nodes_[i];
            auto trail = trails_[i];

            trail = std::max(min_pheromone_value, trail * (1 - evaporation_rate));
            trail = (node == node_a || node == node_b)
                  ? std::min(max_pheromone_value, delta + trail)
                  : trail;
    ...
    void update_default_pheromone_level(double evaporation_rate, double min_pheromone_value) {
        default_pheromone_value_ = std::max(default_pheromone_value_ * (1 - evaporation_rate),
                                            min_pheromone_value);
    }
