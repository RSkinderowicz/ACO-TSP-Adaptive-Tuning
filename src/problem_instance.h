...
enum EdgeWeightType { EUC_2D, EXPLICIT, GEO, ATT, CEIL_2D, GEOM };
...
inline int32_t geom_distance(const Vec2d &p1, const Vec2d &p2)
{
    ...
    longi = Pi * p1.y_ / 180.0;
    longj = Pi * p2.y_ / 180.0;
    ...
    return static_cast<int32_t>(6378388.0 * atan2(sqrt(q1 * q1 + q2 * q2), q5) + 1.0);
}
...
struct ProblemInstance {
    ...
    std::vector<double> all_nn_dist_;  // Distances to nearest neighbors of
                                       // each vertex
    ...
    void compute_nn_lists(uint32_t nn_count) {
        ...
        all_nn_dist_.resize(dimension_ * total_nn_per_node_);
        ...
        // Compute distances to nn lists for each node
        #pragma omp parallel for
        for (uint32_t node = 0; node < dimension_; ++node) {
            const auto offset = node * total_nn_per_node_;
            for (uint32_t i = 0; i < total_nn_per_node_; ++i) {
                const auto nn = all_nearest_neighbors_[offset + i];
                all_nn_dist_[offset + i] = get_distance(node, nn);
            }
        }
    }
    ...
    double get_distance_to_nn(uint32_t from, uint32_t nn_idx) const {
        assert((from < dimension_));
        assert(nn_idx < total_nn_per_node_);

        const auto offset = from * total_nn_per_node_ + nn_idx;
        assert(offset < all_nn_dist_.size());

        return all_nn_dist_[offset];
    }
