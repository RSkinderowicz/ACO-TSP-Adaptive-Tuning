...
ProgramOptions parse_program_options(int argc, char *argv[]) {
    ...
    p.add("x", "EXtra param. with meaning depending on the alg.", opts.x_);

    p.add("reward-policy", "Policy used in the MAB", opts.reward_policy_);

    p.add("mab", "Algorithm to solve MNE-rel. MAB problem", opts.mab_);
