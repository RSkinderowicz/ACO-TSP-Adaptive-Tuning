...
Limits calc_trail_limits_x(uint32_t /*dimension*/,
                            uint32_t cand_list_size,
                            double p_best,
                            double rho,
                            double solution_cost) {
    const auto tau_max = 1 / (solution_cost * (1. - rho));
    const auto avg = 2 * cand_list_size;  // This is far smaller than dimension/2
    const auto p = pow(p_best, 1. / avg);
    const auto tau_min = min(tau_max, tau_max * (1 - p) / ((avg - 1) * p));
    return { tau_min, tau_max };
}
...
public:
    ...
    double deposit_pheromone(const std::vector<uint32_t> &route, double cost) {
        const double deposit = 1.0 / cost;
    ...
    double par_deposit_pheromone(const std::vector<uint32_t> &route, double cost) {
        const double deposit = 1.0 / cost;
        auto &pheromone = get_pheromone();

        #pragma omp for
        for (size_t i = 0; i < route.size(); ++i) {
            auto prev = i > 0 ? route[i-1] : route.back();
            auto node = route[i];
            pheromone.increase(prev, node, deposit, trail_limits_.max_);
        }
        return deposit;
    }
};
...
void compress_route(const vector<uint32_t> &route,
                    const vector<bool> &marked,
                    vector<uint32_t> &comp_route,
                    vector<uint32_t> &compressed) {

    const auto route_size = route.size();
    const uint32_t Sentinel = route_size;
    std::fill(compressed.begin(), compressed.end(), Sentinel);
    comp_route.clear();

    uint32_t unmarked_count = 0;
    auto it = route.rbegin();
    while (it != route.rend() && !marked[*it]) {
        ++unmarked_count;
        ++it;
    }
    if (it == route.rend()) return; // Early return if no marked nodes found

    uint32_t prev_marked = *it;
    ...
    for (uint32_t i = 0; i < route_size; ++i) {
        ...
        if (marked[a]) {
            if (unmarked_count) {
                assert(prev_marked != a);
                compressed[prev_marked] = a;
                unmarked_count = 0;
                ++compressed_count;
            }
            prev_marked = a;
            comp_route.push_back(a);
        } else {
            ++unmarked_count;
        }
    }
...
void decompress_route(const vector<uint32_t> &route,
                    const vector<uint32_t> &pos_in_route,
                    const vector<uint32_t> &comp_route,
                    const vector<uint32_t> &compressed,
                    vector<uint32_t> &new_route) {

    const auto route_size = route.size();
    
    new_route.clear();
    auto prev_node = comp_route.back();
    for (auto node : comp_route) {
        if (compressed[prev_node] == node || compressed[node] == prev_node) {
            // Inject compressed fragment from the original route
            auto from = prev_node, to = node;
            if (compressed[prev_node] == node) {
                auto i = pos_in_route[from] + 1 < route_size ? pos_in_route[from] + 1 : 0;
                auto k = pos_in_route[to];
                for (; i != k; i = i + 1 < route_size ? i + 1 : 0) {
                    new_route.push_back(route[i]);
                }
            } else {
                swap(from, to);
                auto i = pos_in_route[to] > 0 ? pos_in_route[to] - 1 : route_size - 1;
                auto k = pos_in_route[from];
                for (; i != k; i = i > 0 ? i - 1 : route_size - 1) {
                    new_route.push_back(route[i]);
                }
            }
        }
...
par_build_initial_routes(const ProblemInstance &problem,
    ...
    if (use_local_search) {
        #pragma omp parallel for default(none) shared(problem, routes, nn_count, sol_count, std::cout)
        for (uint32_t i = 0; i < sol_count; ++i) {
...
uint32_t select_next_node(const Pheromone_t &pheromone,
    ...
    for (auto node : nn_list) {
        ...
        cl[cl_size] = node;
        auto prod = valid 
                   ? heuristic.get(current_node, node) * pheromone.get(current_node, node)
                   : 0.0;
        cl_products_sum += prod;
        cl_product_prefix_sums[cl_size] = cl_products_sum;
        cl_size += valid;
...
public:
    ...
    Route(std::vector<uint32_t> route, CostFunction fn, double cost = 0)
        : route_(route),
          cost_fn_(fn),
          cost_(cost)
    {
...
run_mfaco(const ProblemInstance &problem,
    ...
    #pragma omp parallel default(shared)
    {
        ...
        for (int32_t iteration = 0 ; iteration < iterations ; ++iteration) {
            ...
            // the same path -- i.e. if we run the program with the same PRNG
            // seed (--seed X) then we get exactly the same results.
            #pragma omp for schedule(static, 1) reduction(+ : ant_sol_updates, total_new_edges)
            for (uint32_t ant_idx = 0; ant_idx < ants.size(); ++ant_idx) {
...
class RewardPolicy {
public:
    virtual double get_reward(double cost_before, double cost_after) const = 0;
};

struct ConstReward : public RewardPolicy {
    double value_;
public:
    ConstReward(double value = 1) : value_(value) {}

    double get_reward(double cost_before, double cost_after) const override {
        return (cost_after < cost_before) ? value_ : 0;
    }
};

struct ProportionalReward : public RewardPolicy {
    double scaling_factor_;
public:
    ProportionalReward(double scaling_factor): scaling_factor_(scaling_factor) {}

    double get_reward(double cost_before, double cost_after) const override {
        return (cost_after < cost_before)
            ? std::min(1., (cost_before - cost_after) / cost_before * scaling_factor_)
            : 0;
    }
};

/* A version of the Focused ACO in which Multi-Armed Bandit based policy is used to 
set the value of the MNE (min. number of new edges) parameter.

The Exp3 seems to work best across a range of TSPLIB instances.
However, other policies are available (see mab.h)
 */
template<typename ComputationsLog_t>
std::unique_ptr<Solution> 
run_faco_apt(const ProblemInstance &problem,
             const ProgramOptions &opt,
             ComputationsLog_t &comp_log) {
    const auto dimension  = problem.dimension_;  
    const auto cl_size    = opt.cand_list_size_;
    ...
    Route best_ant { start_route, problem.get_distance_fn(), initial_cost };

    vector<Route> ant_solutions(ants_count);

    Route source_solution{ best_ant };
    Route *source_sol_ptr = nullptr;
    ...
    int default_num_threads = omp_get_max_threads();
    using namespace MultiArmedBandit;

    std::unique_ptr<Policy> mab_oracle; // multi-armed bandit oracle

    for (int i = 0; i < default_num_threads; ++i) {
        if (opt.mab_ == "ucb1tuned") {
            mab_oracle = ::make_unique<UCB1Tuned>(5u);
        } else if (opt.mab_ == "uniform") {
            mab_oracle = ::make_unique<Uniform>(5u);
        } else if (opt.mab_ == "rc") {
            mab_oracle = ::make_unique<RC>(5u);
        } else if (opt.mab_ == "eg") {
            mab_oracle = ::make_unique<EpsilonGreedy>(5u);
        } else if (opt.mab_ == "exp3") {
            mab_oracle = ::make_unique<Exp3>(5u);
        } else if (opt.mab_ == "exp3p") {
            mab_oracle = ::make_unique<Exp3P>(5u, iterations / static_cast<double>(default_num_threads));
        } else {
            std::cerr << "Unknown MAB policy : " << opt.mab_ << std::endl;
            abort();
        }
    }

    std::shared_ptr<RewardPolicy> reward_policy;
    if (opt.reward_policy_ == "proportional") {
        reward_policy = make_shared<ProportionalReward>(dimension * 0.1);
    } else if (opt.reward_policy_ == "constant") {
        reward_policy = make_shared<ConstReward>(0.1);
    } else {
        std::cerr << "Unknown reward policy: " << opt.reward_policy_ << std::endl;
        abort();
    }
    ...
    #pragma omp parallel default(shared)
    {
        ...
        vector<uint32_t> ls_checklist;
        ls_checklist.reserve(dimension);
        auto *oracle = mab_oracle.get();

        for (int32_t iteration = 0 ; iteration < iterations ; ++iteration) {
            #pragma omp barrier

            source_sol_ptr = &source_solution;
            ...
            // the same path -- i.e. if we run the program with the same PRNG
            // seed (--seed X) then we get exactly the same results.
            #pragma omp for schedule(dynamic, 1) reduction(+ : ant_sol_updates, total_new_edges, local_source_sol_updates)
            for (uint32_t ant_idx = 0; ant_idx < ants_count; ++ant_idx) {
                auto target_new_edges = opt.min_new_edges_;

                #pragma omp flush   // Update our memory to see writes made in other threads

                const Route *source = source_sol_ptr;

                Route &route = ant_solutions[ant_idx];
                route = *source;  // Copy source solution into route

                auto start_node = get_rng().next_uint32(dimension);
                auto start_cost = route.cost_;
                ...
                auto sel_arm = oracle->pull();
                target_new_edges = 4 + sel_arm;
                //target_new_edges = 4 + get_rng().next_uint32(5);
                ...
                while (new_edges < target_new_edges && visited_count < dimension) {
                    ...
                    if (!source->contains_edge(curr, sel)) {
                        /*
                        For simplicity and efficiency, we are looking only at
                        the (curr, sel) edge even though the relocation could
                        have created additional new edges.
                ...
                if (opt.count_new_edges_) {  // How many new edges are in the new sol. actually?
                    total_new_edges += count_diff_edges(route, *source);
                }
                ...
                if (opt.source_sol_local_update_ && route.cost_ < source->cost_) {

                    #pragma omp flush  // Show our route to the other threads

                    if (route.cost_ < source_sol_ptr->cost_) {  // Is our sol. better?

                        #pragma omp atomic write
                        source_sol_ptr = &route;
                    } 

                    ++local_source_sol_updates;
                }

                oracle->update(sel_arm,
                               reward_policy->get_reward(start_cost, route.cost_));
            }

            Route *iteration_best = &ant_solutions.front();

            #pragma omp master
            {
                auto sol_costs_it = sol_costs.begin();
                for (auto &sol : ant_solutions) {
                    if (sol.cost_ < iteration_best->cost_) {
                        iteration_best = &sol;
                    }
                    *sol_costs_it = sol.cost_;
                    ++sol_costs_it;
                }
                if (iteration_best->cost_ < best_ant.cost_) {
                    best_ant =*iteration_best;
            ...
            #pragma omp master
            {
                ...
                // Increase pheromone values on the edges of the new
                // source_solution
                model.deposit_pheromone(update_ant.route_, update_ant.cost_);

                pher_deposition_time += omp_get_wtime() - start;

                source_solution = update_ant;
            }
        }
    }

    cout << "Oracle stats: \n";
    mab_oracle->print_summary();
    comp_log("mab arm pulls", mab_oracle->get_arm_pulls());
    comp_log("mab arm rewards", mab_oracle->get_arm_rewards());
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        } else if (args.algorithm_ == "faco_apt") {
            alg = run_faco_apt;
        ...
        for (int i = 0 ; i < args.repeat_ ; ++i) {
            ...
            if (!problem.is_route_valid(result->route_)) {
                std::cerr << "Invalid final solution!" << std::endl;
                abort();
            }
            const auto true_cost = problem.calculate_route_length(result->route_);
            if (std::fabs(true_cost - result->cost_) > 1e-3) {
                std::cerr << "Invalid final solution cost: " << result->cost_ 
                          << " instead of " << true_cost << std::endl;
                abort();
            }
